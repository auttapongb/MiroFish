...
class PanoramaResult:
    ...
    def to_text(self) -> str:
        """转换为文本格式（完整版本，不截断）"""
        text_parts = [
            f"## Panorama Search Results (Future View)",
            f"Query: {self.query}",
            f"\n### Statistics",
...
class ZepToolsService:
    ...
    def _generate_sub_queries(
        ...
        """
        system_prompt = """You are an expert problem analyst. Your task is to decompose a complex question into multiple sub-questions that can be independently observed in the simulation world.
...
4. Write all sub-questions IN ENGLISH.
5. Return JSON format: {"sub_queries": ["sub-question 1", "sub-question 2", ...]}"""
...
Please decompose the following question into {max_queries} sub-questions, written IN ENGLISH:
{query}
...
Return a JSON list of sub-questions."""
    ...
    def interview_agents(
        ...
        INTERVIEW_PROMPT_PREFIX = (
            ...
            "answer the following questions directly in plain text. IMPORTANT: respond ENTIRELY in English.\n"
            "Requirements:\n"
            ...
            "4. Answer each question by number, prefixing each with \"Question X:\" (X is the question number)\n"
        ...
        try:
            ...
            # FIX: 模拟结束后进程可能已退出，live interview 会失败；此时回退到
            # 持久化人设采访（与 API 层 /api/simulation/interview 的降级逻辑一致）。
            if SimulationRunner.check_env_alive(simulation_id):
                ...
                api_result = SimulationRunner.interview_agents_batch_persisted(
                    simulation_id=simulation_id,
                    interviews=interviews_request,
                    platform=None
                )
    ...
    def _select_agents_for_interview(
        ...
        system_prompt = """You are an expert interview planner. Based on the interview requirement, select the most suitable agents to interview from the simulation agent list.

Selection criteria:
1. The agent's identity/profession is relevant to the interview topic
2. The agent likely holds a distinctive or valuable perspective
3. Choose diverse perspectives (supporters, opponents, neutrals, professionals, etc.)
4. Prioritise roles directly involved in the event
...
Please select at most {max_agents} of the most suitable agents and explain your reasoning."""
        ...
        try:
            ...
            return selected, indices, "Used default selection strategy"
    ...
    def _generate_interview_questions(
        ...
        system_prompt = """You are a professional journalist/interviewer. Based on the interview requirement, generate 3-5 in-depth interview questions IN ENGLISH.
...
Please generate 3-5 interview questions in English."""
    ...
    def _generate_interview_summary(
        ...
        system_prompt = f"""You are a professional news editor. Based on the interviewees' answers, generate an interview summary IN ENGLISH.

Summary requirements:
1. Extract each side's main viewpoints
2. Note points of consensus and disagreement
