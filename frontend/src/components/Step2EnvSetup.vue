...
const autoGeneratedRounds = computed(() => {
  ...
  // 确保至少 1 轮；不再强制最小 40 轮（尊重确定性时长配置）
  return Math.max(calculatedRounds, 1)
})

// 配置加载后，将自定义轮数默认值同步为自动生成轮数（与确定性时长一致）
let customRoundsSynced = false
watch(autoGeneratedRounds, (val) => {
  if (val && !customRoundsSynced) {
    customMaxRounds.value = val
    customRoundsSynced = true
  }
})
